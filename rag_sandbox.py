...
DEFAULT_EMBED_MODEL = os.getenv("EMBED_MODEL", 'C:\Public\workspace\multilingual-e5-base_light')
DEFAULT_VECTOR_PKL = os.getenv("VECTOR_PKL", "chatbot-models/vectordb_docling_v2/chunks.pkl")
DEFAULT_FAISS_INDEX = os.getenv("FAISS_INDEX", "chatbot-models/vectordb_docling_v2/index.faiss")
...
class OllamaClient:
    ...
    def generate(self, prompt: str, system: Optional[str] = None, options: Optional[Dict[str, Any]] = None, stop: Optional[List[str]] = None) -> str:
        ...
        payload = {"model": self.model, "prompt": prompt, "stream": False,"num_thread": 8}
    ...
    def stream_generate(self, prompt: str, system: Optional[str] = None, options: Optional[Dict[str, Any]] = None, stop: Optional[List[str]] = None):
        ...
        url = f"{self.host}/api/generate"
        payload = {"model": self.model, "prompt": prompt, "stream": True,"num_thread": 8}
        if system: payload["system"] = system
        if options: payload["options"] = options
        if stop: payload["stop"] = stop
        with requests.post(url, json=payload, timeout=self.timeout, stream=True) as r:
            ...
            for line in r.iter_lines():
                ...
                if "response" in msg:
                    yield msg["response"]
        logger.info("✅ stream_generate généré en %.2fs", time.perf_counter() - t0)
...
class RAGEngine:
    ...
    def retrieve_context_hybrid(self, question: str, top_k: int = 8, alpha_lexical: float = 0.35, use_rrf: bool = True, mmr_lambda: float = 0.6, pool_size: int = 64):
        ...
        logger.info(f"✅ final_scores: {final_scores}")
        context = "\n\n".join(n.get_content() for n in final_nodes)
        #logger.info(f"✅ Context : {len(context)}")
        logger.info(f"✅ Taille du Context {len(context)}")
        logger.info("✅ Context généré en %.2fs", time.perf_counter() - t0)
...
if __name__ == "__main__":
    ...
    p = argparse.ArgumentParser(description="RAG Hybrid + Ollama (drop-in) — CLI")
    ...
    # NEW: separate embedding model (HF) vs LLM model (Ollama)
    p.add_argument("--embed_model", type=str, default=os.getenv("EMBED_MODEL", DEFAULT_EMBED_MODEL), help="Embedding model for retrieval (HF, e.g., intfloat/multilingual-e5-base)")
    p.add_argument("--model_name", type=str, default=os.getenv("LLM_MODEL", "phi3:mini"), help="LLM model name for Ollama generation")
    ...
    # Modes
    p.add_argument("--preview", type=str, default=None, help="Single question to run retrieval+generation on")
    p.add_argument("--questions_file", type=str, default=None, help="Path to a text file with one question per line")
    p.add_argument("--output", type=str, default=None, help="Output JSONL file for batch mode (defaults to runs/batch_YYYYMMDD-HHMMSS.jsonl)")
   
    args = p.parse_args()
    #print(args.questions_file)
    rag = RAGEngine(
        model_name=args.model_name,            # LLM (Ollama)
        ollama_host=args.ollama_host,
        vector_path=args.vector_path,
        index_path=args.index_path,
        embed_model_name=args.embed_model,     # HF embeddings
    )
    ...
    if args.questions_file:
        ...
        outpath = Path(args.output) if args.output else outdir / f"batch_{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}.jsonl"
        count = 0
        with open(infile, "r", encoding="utf-8") as f_in, open(outpath, "w", encoding="utf-8") as f_out:
            for line in f_in:
                q = line.strip()
                if not q:
                    continue
                ...
                rec = {"question": q, "answer": ans, "context": ctx}
                f_out.write(json.dumps(rec, ensure_ascii=False) + "\n")
                count += 1
        print(f"Wrote {count} results to {outpath}")
        sys.exit(0)

    print("Nothing to do. Use --preview 'your question' or --questions_file path.")


# ------------------ CLI (preview + batch, extended) ------------------
if __name__ == "__main__":
    import argparse, json, sys
    ...
    # NEW: separate embed vs LLM
    p.add_argument("--embed_model", type=str, default=os.getenv("EMBED_MODEL", "intfloat/multilingual-e5-base"), help="HF embedding model for retrieval")
    p.add_argument("--model_name", type=str, default=os.getenv("LLM_MODEL", "phi3:mini"), help="Ollama LLM model for generation")
    p.add_argument("--ollama_host", type=str, default=os.getenv("OLLAMA_HOST", "http://localhost:11434"), help="Ollama host URL")
    p.add_argument("--ollama_timeout", type=int, default=int(os.getenv("OLLAMA_TIMEOUT", "600")), help="Ollama HTTP timeout (s)")
    ...
    # NEW: retrieval-only
    p.add_argument("--no_generate", action="store_true", help="Skip LLM generation; only retrieve and save context")
    ...
    # update client timeout if present
    if rag.ollama_client:
    ...
    if args.preview:
        ...
        if not args.no_generate:
            ...
            try:
                ...
                print(f"[Generation skipped: {type(e).__name__} - {e}]", file=sys.stderr)
        else:
            print("\n(no_generate set — skipping LLM)")
        sys.exit(0)
    ...
    if args.questions_file:
        ...
        with open(infile, "r", encoding="utf-8") as f_in, open(outpath, "w", encoding="utf-8") as f_out:
            ...
            for line in f_in:
                ...
                if not args.no_generate:
                    try:
                        rec["answer"] = rag.generate(q, ctx)
                    except (ReadTimeout, ReqConnError) as e:
                        rec["answer_error"] = f"{type(e).__name__}: {e}"
                        rec["answer"] = ""
                else:
                    rec["answer"] = ""
                f_out.write(json.dumps(rec, ensure_ascii=False) + "\n")
