...
import os
os.environ["HF_HOME"]=r"C:\Users\RKONAN\.cache\huggingface"
...
def run(cfg: dict):
    ...
    pipeline_options.do_ocr = True  # <-- désactive l’OCR
    pipeline_options.ocr_options =EasyOcrOptions(lang=['fr','en'],
                                                
                                                 model_storage_directory=r"C:\Users\RKONAN\.EasyOCR\model",
    ...
    #pipeline_options.page_image_dpi=220
    logger.info("Chargement du  DocumentConverter")
    ...
    tokenizer =AutoTokenizer.from_pretrained(embed_model_name,local_files_only=True,revision="v2.0")
    logger.info("Chargement du  model")
    model=AutoModel.from_pretrained(embed_model_name,local_files_only=True,revision="v2.0")
    logger.info("Chargement du  HybridChunker")
    chunker = HybridChunker(tokenizer=tokenizer,model=model,max_tokens=500)
