...
def generate_html_report(stats: dict, per_file_rows: List[dict], out_path: Path):
    ...
    """
    # simple histogram bins for chunk lengths
    lens = stats.get("chunk_lengths", [])
    if not lens and per_file_rows:
        # reconstruct rough lens from totals:
        pass
    # Build HTML
    html_parts = []
    ...
    # Per-file table (top 200 by chunks_kept desc)
...
def run(cfg: dict):
    ...
    stats = {
        ...
        "chunk_lengths": lens[:10000],  # cap for report
    }
    ...
    print(f"[OK] Rapport HTML: {report_html}")

def main():
    ap = argparse.ArgumentParser(description="Step6 Docling: regex excludes + HTML report")
